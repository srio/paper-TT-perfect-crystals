...
\end{equation}
\inblue{thus giving
\begin{subequation}\label{eq:approxDeltaPsi}
\begin{align}
    \Delta\Psi + k^2\Psi= 
...
% \footnote{We discarded the terms $\chi_h D_h \exp(i (\textbf{k}_0+2\textbf{h}) . \textbf{r}) + \chi_{-h} D_0 \exp(i (\textbf{k}_0 - 2 \textbf{h}) .\textbf{r})$,in such a way that the two-beam approximation is maintained.}
\begin{equation}
...
\hat{\textbf{n}}.\textbf{s}_t=0$.
The equation of the crystal surface is $\gamma_0 s_0 + \gamma_h s_h =0$, with $\gamma_{0,h}=\textbf{n}.\textbf{s}_{0,h} \equiv \cos(\theta_{0,h})$ the director cosines with respect to \inblue{$\textbf{n}$, the unit inward normal vector to the entrance plane surface}.

% \todo{ REMOVE or MOVE? For any point $\textbf r=(s_0,s_h,s_t)$ inside the crystal, we introduce the path length $s$ of the incident ray going in this point: this ray enters the crystal at the point of coordinates $(s'_0,s_h,s_t)$ such that $\gamma_0 s'_0+\gamma_h s_h = 0$, so that 
% \begin{equation}
% \label{eq:s}
...
\begin{subequation}\label{eq:BdefinitionNN}
\begin{align}
D_{0,h}(s) =& 
...
\end{align}
\end{subequations}
or, in terms of $D_{0,h}(s)$ [equation~\inred{(15)}]
% (\ref{eq:BdefinitionNN})]
...
\inblue{They also correspond to the two branches of the dispersion surface}. 
The real part of the argument of the exponential factors, $-T [\operatorname{Im} u_0 (b+1)/2  \pm \operatorname{Im} a]$, \inblue{is related to the absorption}.
% The factor called by convention ``normal absorption" is represented as $\exp[-T \operatorname{Im} u_0 (b+1)/2 ]$.
\inblue{When $\operatorname{Im}a<0$,
the absorption is less than $\exp[-T \operatorname{Im} u_0 (b+1)/2 ]$ for the first matrix, and more than that for the second one.} Similarly, for  $\operatorname{Im}a \ge 0$ the matrices present opposite behavior.  If $T |\operatorname{Im}a|$ is large (for example $T |\operatorname{Im}a|  \gtrsim 5$), we can keep only one term in equation~(\ref{eq:Mfactorized}):
...
\begin{figure}\label{fig:braggMap}
    ...
    c) Laue $|D_h|^2$, d) Laue $|D_0|^2$. \todo{CHANGE UNITS TO $\mu$rad}
    }
\end{figure}
...
\begin{equation}\label{eq:thickbraggr}
    ...
    \end{cases}
\end{equation}
Both equations are equivalent \inblue{assuming that the sign in $a=\pm \sqrt{a^2}$ is correctly selected}. The condition on $\operatorname{Im}(a)$ in equations~(\ref{eq:thickbraggr}) is in accordance with the physical condition that $|r_B|$ go to zero when $|\sin\theta-\sin\theta_c|$ is large.
