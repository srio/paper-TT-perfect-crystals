...
\\
 ...
 \\
	z &= \frac{1-b}{2} \Psi_0 + \frac{b}{2} \alpha 
 \\
	\alpha &= \frac{1}{|\vec{k}_0|^2} 
              \left[ |\vec{B_H}|^2 +
...
It is easy to see that $x_2-x_1=2 X / (P\Psi_{\bar{H}})$, $x_1 x_2 = -b \Psi_H/(\Psi_{\bar{H}} \inred{/P^2})$, and 
\begin{equation}
  c_{1,2} = \exp
...
and
\begin{equation}
    x_2 c_1 - x_1 c_2 = \frac{c}{P \Psi_{\bar{H}}} \left[ 
 -(X+z)e^{im}-(X-z)e^{-im}\right] =
 \frac{2 c}{P \Psi_{\bar{H}}}(-X \cos(m) - i z \sin(m)).
\end{equation}
Replacing in equation~(\ref{eq:ZacBragg}) the terms obtained here  we finally get:
	\begin{subequations}
	\label{eq:ZacBragg2}
    \begin{align}
	r^Z_{L} &=  i c b P \Psi_H \frac{\sin(m)}{X}, \\
...
\begin{equation}
    x_2 c_2 - x_1 c_1 = \frac{c}{P \Psi_{\bar{H}}} \left[ 
 -(X+z)e^{-im}-(X-z)e^{im}\right] =
 \frac{2c}{P \Psi_{\bar{H}}}(-X \cos(m) + z \sin(m)),
\end{equation}
that introduced in equation~(\ref{eq:ZacBragg}) we finally get:
	\begin{subequations}
	\label{eq:ZacLaue2}
    \begin{align}
	r^Z_{B} &=  i b \Psi_H \frac{\sin(m)}{z \sin(m) - X \cos(m)}, \\
 	t^Z_{B} &= \frac{ c X}{X \cos(m) - z \sin(m)}.   
    \end{align}
    \end{subequations}
