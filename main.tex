...
We will now obtain the solutions of the TT equations~(\ref{eq:TTcompact}) depending on the single variable $s=s_0+s_h \gamma_h/\gamma_0$, which means 
...
The determinant of the matrix $M$ is equal to one for a non-absorbing crystal (in this case $u_0$ and $\omega$ are real).
...
The reflecting power, or diffraction profile, often referred as rocking curve, is obtained from equations~(\ref{eq:lauerandt}) as $\mathcal{R}(\theta)=|r(\theta)|^2 P$, with $P=1/|b|$ the power factor (see \cite{ZachariasenBook} pag. 122). The forward diffracted profile (or transmitted) is $\mathcal{T}=|t|^2$. There are called hereafter reflectance ($\mathcal{R}$) and transmittance ($\mathcal{T}$). An example is in Fig.~\ref{fig:laueProfiles}. 
...
The diffraction profiles are $\mathcal{R}=|r|^2 P$ (diffracted \inblue{power} or reflectance) and $\mathcal{T}=|t|^2$ (forward diffracted \inblue{power} or transmittance).
...
\textit{3.4.1  Reflection amplitude for a thick crystal in Bragg case}

In case of thick (or semi-infinite) Bragg crystal, the reflection amplitude is given by $r_B=-m_{21}/m_{22}$ which from equation~(\ref{eq:Mthickapprox}) gives  
...
\textit{3.4.2  Reflection amplitude for a non-absorbing crystals in Bragg case \todo{A REVOIR!!!}}

If the true absorption is zero (``transparent" crystal), $u_{-h}=u^*_h$, and both $\omega$ [equation~(\ref{eq:omega})] and $a^2$ [equation~(\ref{eq:a})] are real. We have $a^2=\omega^2-|b|u_u u^*_{h}$ from which it is convenient to define (like the variables in \cite{ZachariasenBook})
\begin{subequation}\label{eq:uandy}
\begin{align}
    u = \sqrt{|b| u_h u_h^*}, \\
    y = \frac{\omega}{u}, \\
    A = T u, \\
    u_h=|u_h| e^{i\varphi_h}, 
\end{align}
\end{subequation}
hence $a^2=u^2(y^2-1)$.
For getting the final expressions of the reflectance, it is convenient to consider separately the cases i) $|y|\le 1$, for which 
$a=\pm i u \sqrt{1-y^2}$, 
and ii) $|y|>1$, with 
$a=\pm u \sqrt{y^2-1}$. 
% $i\sin(aT)=\mp \sinh(A\sqrt{1-y^2})$, 
% For the first case 
% $a=\pm i u \sqrt{1-y^2}$, $i\sin(aT)=\mp \sinh(A\sqrt{1-y^2})$, 
% $\cos(aT)=\cosh(A\sqrt{1-y^2})$,
% $-b u_h/u=|b|^{1/2}$.
% For the second case


\todo{REMOVE?
With these consideration we get from equation~(\ref{eq:braggrandt}a),

\begin{equation}\label{eq:rBnoabsorption}
    r_B^{\text{transparent}} = \begin{cases} 
    \frac{|b|^{1/2} e^{i\varphi_h} \sinh(A\sqrt{1-y^2})}{y \sinh(A\sqrt{1-y^2})-i\sqrt{1-y^2} \cosh(A\sqrt{1-y^2})}
    & \text{if  $|y| \le 1$},\\
    \frac{i e^{i\varphi_h} |b|^{1/2} \sin(A\sqrt{y^2-1})}{\sqrt{y^2-1} \cos(A\sqrt{y^2-1}) + i y \sin(A \sqrt{y^2-1})}
    & \text{if $|y|>1$}.
    \end{cases}
\end{equation} 
}

For the thick non-absorbing crystals, equations~(\ref{eq:thickbraggr}) are, with the new notation \todo{CHECK}:

\begin{equation}\label{eq:rBnoabsorptionThick}
    r_B^{\text{transparent, thick}} = \begin{cases} 
    |b|^{1/2} e^{i \varphi_h} (y + i \sqrt{1-y^2})
    & \text{if  $|y| \le 1$},\\
    |b|^{1/2} e^{i \varphi_h}  ( y - \sqrt{y^2-1} )
    & \text{if $|y|>1$}.
    \end{cases}
\end{equation} 
The reflected power or reflectance is [from equation~(\ref{eq:DarwinGeneral})]

\begin{equation}\label{eq:Darwin}
\mathcal{R}_B^{\text{transparent, thick}} = 
    \frac{\omega+a}{\omega-a} =
    \begin{cases} 
    1
    & \text{if  $|y| \le 1$},\\
    ( y - \sqrt{y^2-1} )^2
    ...
    \end{cases}
\end{equation}
If using the Ewald recipe [equation~(\ref{eq:EwaldGeneral})]
\begin{equation}\label{eq:Ewald}
\mathcal{R}_B^{\text{Ewald}} = 
\frac{\omega + a}{\omega} = 
\begin{cases} 
    1
    & \text{if  $|y| \le 1$},\\
...
% let us consider the limit of the equations~(\ref{eq:rBnoabsorption}) for $A \rightarrow \infty$.

% i) For $|y|\le 1$, the limit is 
% \begin{equation}\label{eq:rBnoabsorptionthickCenter}
% r_B^{\text{transparent,thick}} = |b|^{1/2} e^{i \varphi_h} (y + i \sqrt{1-y^2}).
% \end{equation}
% \todo{Note that the same result is obtained using equation~(\ref{eq:thickbraggr})}.
% From here, the reflecting power is $\mathcal{R}^{\text{transparent,thick}}=|r_B^{\text{transparent,thick}}|^2/|b|=1$, indicating total reflection in this zone. 

% ii) in the zone $|y|>1$ the reflection amplitude from equation~(\ref{eq:rBnoabsorption}) [as well as the most generic equation ~(\ref{eq:braggrandt})a] contain periodic functions of an argument including $T$. Therefore they do not have a well-defined limit, in the common sense, for $T\rightarrow \infty$. The limit is then defined as the zero-order term of the Fourier series of the considered periodic function (or, equivalently, the average of this function over one period). Considering the function $f(x)= i \sin(x) / (K \cos x + i L \sin x)$ has average (zeroth order of the Fourier series) $\bar{f(x)}=(L-K)^{-1}$ (see Appendix XX), we obtain from equation~(\ref{eq:rBnoabsorption})
% \begin{equation}\label{eq:rBnoabsorptionthickTails}
% r_B^{\text{transparent,thick}} = |b|^{1/2} e^{i \varphi_h}  ( y - \sqrt{y^2-1} ).
% \end{equation}
% From here, the reflecting power is $\mathcal{R}^{\text{transparent,thick}}=r_B^{\text{transparent,thick}} (r_B^{\text{transparent,thick}})^{^*}/|b|=(y-\sqrt{y^2-1})^2$,  \todo{$|y|$??}, known as the Darwin solution since it was deduced by C. G. Darwin. 

% A physical explanation of the above procedure can be the following. The incident wave, even considered as strictly monochromatic, is never a plane wave with infinite lateral dimension, but a sort of ``quasi-plane" wave. A small variation of the angle of incidence around its nominal value $\theta$ would always exist. This would imply small variations of the parameter $A$ (or $a$ for equations~(\ref{eq:braggrandt})), which is a function of $\theta$. We are interested in a periodic function of the variable $A\sqrt{1-y^2}$ (or $aT$). The variation of this variable becomes arbitrarily large as $T \rightarrow \infty$. Consequently, all the terms in the Fourier series are averaged to zero, except the zeroth order. 

% The other classical solution of the non-absorbing Bragg thick crystal is the so-called Ewald solution. This is calculated as

% $\mathcal{R}^{\text{Ewald}}=
% \overline{
% r_B^{\text{transparent}} (r_B^{\text{transparent}})^{^*}
% }
% /|b|$, therefore averaging the power instead of the amplitudes (in Darwin). Considering that $\overline{f(x) f^*(x)}=[(L-K)L]^{-1}$ (see Appendix XX), we obtain the usual Ewald solution  $\mathcal{R}^{\text{Ewald}}= $.
...
In many applications, like in ray tracing, it is essential to know the output wavevector $\textbf{k}_{\text{out}}$ that exits from the crystal. 

This direction of the Bragg \todo{also Laue?} diffracted wave can be written on the form 
\begin{equation}
    \textbf{k}_{h,\text{out}} = \textbf{k} + \beta \textbf{n}, \nonumber
\end{equation}
with $\textbf{n}$ the unit vector along the normal \todo{inwards?} to the crystal surface. The \inred{(real)} coefficient $\beta$ is obtained by writing that the modulus of $\textbf{k}_{h,\text{out}}$ is equal to $k$
\begin{equation}
    |\textbf{k}_{h,\text{out}}|^2 = |\textbf{k}_h + \beta \textbf{n}|^2. \nonumber
\end{equation}
Note that $|\textbf{k}_h|^2=k^2(1-\alpha)$ and $\textbf{k}_h . \textbf{n} = k \gamma_h$, from which it is obtained the equation
\begin{equation}
