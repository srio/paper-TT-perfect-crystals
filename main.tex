...
\inred{Our formulation is written for the case of $\sigma$-polarization. For the case of $\pi$-polarization, it is sufficient to replace $\chi_h$ and $\chi_{-h}$ by $C\chi_h$ and $C\chi_{-h}$ with $C=\cos(2\theta_B)$.}
...
\end{equation}
Its solution \inred{satisfying} the boundary conditions $D_0^{\text{ref}}=1$ for  $\gamma_0 s_0 + \gamma_h s_h =0$ (equation of the crystal surface) is $D_0^{\text{ref}}= \exp[i u_0 (s_0 + s_h \gamma_h/\gamma_0)]\inred{=\exp(i u_s s)}$ \inred{where}
...
\end{align}
\end{subequations}
\inred{It is convenient to use} the functions $B_{0,h}(s)$ by setting
...
\begin{equation}\label{eq:omega}
    ...
    \inred{\omega=\frac{ b (u_0 + \alpha') - u_0}{2}}.
\end{equation}
\inred{Equations}~(\ref{eq:TTinD}) become 
\begin{subequations}
\label{eq:TTinB}
\begin{align}
...
% \end{equation}
% }
\inred{From} the case $s=0$, we obtain
\begin{equation}
\label{eq:cs}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Expressions of  $\alpha$, $\omega$ and $a$ as function\inred{s} of \inred{the} angles}
\label{sec:physical_meaning}
%%%%%%%%%% alpha
% Expressing $\alpha$ [equation~(\ref{eq:alpha})] as a function of the angles, we note 
\inred{Note} that $h=2k \sin\theta_B$, being $\theta_B$ the geometrical Bragg angle, and $\textbf{k}_0 . \textbf{h}= -k h \sin\theta$, with $\theta$ the glancing angle of $\textbf{k}_0$ on the reflecting planes. \inred{From equation~(\ref{eq:alpha}) we obtain}
\begin{equation}\label{eq:alphavsangles}
    \alpha=4\sin\theta_B (\sin\theta-\sin\theta_B) \inred{\approx 2 (\theta-\theta_B) \sin (2\theta_B)}.
\end{equation}
\inred{Our definition of $\alpha$ [equation~(\ref{eq:alpha})] was made in such a way that $\alpha$ increases when $\theta$ increases\footnote{Our $\alpha$ has the opposite sign than the $\alpha$ defined in (equation [3.114b]) of \cite{ZachariasenBook}}. The approximated value of $\alpha$ is not valid far from the Bragg position or when $\theta_B$ approaches $\pi/2$ (normal incidence); therefore  equation~(\ref{eq:alpha}) is used in the {\tt crystalpy} software.}
...
%%%%%%%%%% thetac
The ``corrected Bragg angle" $\theta_c$, \inred{that differs from $\theta_B$ because of the effect of refraction}, is obtained as the $\theta$ value such that $\operatorname{Re} \omega=0$, or
...
\end{equation}
\inred{From equations~(\ref{eq:omegavsangles}) and (\ref{eq:correctedBraggAngleExact}), the value of $\omega$ 
...
\begin{equation}\label{eq:Mfactorized}
    ...
    \end{pmatrix}, 
\end{equation}
where the two terms are \inred{interchanged} when $a$ is changed in $-a$. They correspond to the two roots of $a^2$.
They also correspond to the two branches of the dispersion surface \inred{[see e.g.~\cite{authierbook}]}. 
...
\inred{A beam, that would be absorbed without Bragg diffraction if $T \operatorname{Im} u_0 \gg  1$, may partially go through the ``thick crystal" in the condition of Bragg diffraction. Equation~(\ref{eq:Mthickapprox}) is a clear expression of the Borrmann effect.}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Reflection and transmission amplitudes in \inred{Laue case}}
...
\begin{figure}\label{fig:laueProfiles}
    ...
    \caption{Calculated reflectance \inred{(a)} and transmittance \inred{(b)} for a \SI{10}{\micro\meter} thick Laue Si 111 crystal at 8 keV, with 65 degrees of asymmetric angle. The Bragg angle is $\theta_B$=14.31$^\circ$.}
\end{figure}

It is also interesting to consider the case of incidence along the direction of $\textbf{k}_h$ (diffraction vector $-\textbf{h}$), for which ($D_0(0), D_h(0)=(0,1)$. It is directly seen from equation~(\ref{eq:Mtransfer}) that the transmission and reflection amplitudes are $\bar{t}_L=\inred{=D_h(T)=}m_{22}$ and $\bar{r}_L=\inred{D_0(T)=}m_{12}$ (note that the reflection power factor is $P=|b|$ in this case). These results can be written as
...
The exponential factor in equations~(\ref{eq:lauerandt}) gives a damping \inred{factor}, which is (using $u_0 +\omega=[(b+1)u_0+b\alpha']/2$ from equation~({\ref{eq:omega}}), 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Reflection and transmission amplitudes in \inred{Bragg case}. The S-matrix}
...
\begin{figure}\label{fig:braggProfiles}
    ...
    \caption{Calculated reflectance \inred{(a)} and transmittance \inred{(b)} of a symmetrical Bragg Si 111 crystal with \SI{10}{\micro\meter} thickness at 8 keV.}
\end{figure}
...
Fig.~\ref{fig:braggMap}b shows that the penetration of the incident wave inside the crystal is small \inred{in a limited interval around $\theta_c$}.
% with respect to the total crystal thickness. 
% This is related to the penetration length or depth. 
In Fig.~\ref{fig:penetration} we fitted the intensity profile of $|D_0(s)|^2$ versus depth for each value of $\theta-\theta_B$.
The fact that for a thick crystal in Bragg geometry $|D_0(s)|^2$ has significant values only in the vicinity of the crystal surface, in the central region, can be explained from equations~(\ref{eq:bragginside}). The moduli of the functions $\sin(aT-as)$ and $\cos(aT-as)$ are approximately proportional to $\exp[(T-s)|\operatorname{Im}a|]$ if the argument of this exponential function is sufficiently large. Consequently, $|D_0(s)|^2$ is nearly proportional to $\exp[-s(2|\operatorname{Im}a|+\inred{(}b+1\inred{)}\operatorname{Im}u_0)]$. Writing $|D_0(s)|^2=\exp[-s/s_{ext}]$, with $s_{ext}\inred{=[(2|\operatorname{Im}a|+(b+1)\operatorname{Im}u_0)]^{-1}}$ the extinction length (measured along the $s_0$ axis), we obtain for the Bragg symmetric ($b=-1$) case  
...
\end{equation}
Equation~(\ref{eq:tails}) \inred{represents} the tails of the reflection profile. As discussed previously, the sign selection is such that $|r_B|$ tends to zero \inred{for} large values of $|\omega|$. Equation~(\ref{eq:totalreflection}) corresponds to the zone of total reflection. \inred{The reflectance is}
...
% Equation~(\ref{eq:Darwin}) is the so-called Darwin formula for the Bragg reflection. We recall that it was obtained by
...
A first example shows how simple is \inred{the application of} this recipe of multiplication of transfer matrices to get the reflectance of a simple two-layer crystal. Consider
...
In some applications, \inred{as in ray tracing}, it is essential to know the diffracted wavevector $\textbf{k}_{\text{out}}$ that exits from the crystal. As mentioned before, the \inred{choice of $\textbf{k}_0$ in equation~(\ref{eq:wavefield}) is somewhat arbitrary.} 
In our choice, $\textbf{k}_0$ corresponds exactly \inred{to the wavevector} of the incident \inred{plane} wave.
The vector $\textbf{k}_h$ (see section~\ref{sec:TT}), defined  as $\textbf k_h=\textbf k_0 + \textbf h$, does not correspond in general to the wavevector of the outgoing ray or wave outside the crystal. \inred{$\textbf{k}_\text{out}$} has the form 
\begin{equation}
    \textbf{k}_{\text{out}} = \textbf{k}_h + \beta \textbf{n}, \nonumber
\end{equation}
with $\textbf{n}$ the unit vector along the inward normal to the crystal \inred{exit} surface. The (real) coefficient $\beta$ is obtained by writing that the modulus of $\textbf{k}_{\text{out}}$ is equal to $k$:
\begin{equation}
...
\end{equation}
where the $\pm$ sign is chosen in such a way that $\beta=0$ when $\alpha=0$ (i.e., \inred{positive} for $\gamma_h>0$ and \inred{negative} for  $\gamma_h<0$).
...
These classes ({\tt Vector}, {\tt Photon} and {\tt ComplexAmplitudePhoton}) can hold stacks (the internal storage is done with arrays to speed-up vector operations). The {\tt ComplexAmplit\inred{u}dePhoton} classe 
...
Typical angle or photon scans, as shown in Fig.~\ref{fig:braggProfiles}, are calculated defining a $\tt ComplexAmplitudePhoton$ entity for each point, group the\inred{m} in a {\tt ComplexAmplitudePhotonBunch} for then calculate the diffraction by the crystal using {\tt calculateDiffractedComplexAmplitudes}.
% \todo{listing?} 
...
\begin{align}
    ...
    4 \sin\theta_B[\sin(\theta_B+\Delta\theta)-\sin\theta_B]&\inred{=}
    \alpha \nonumber
\end{align}
\end{subequations}
...
The \inblue{$r$} and then the reflected and transmitted amplitudes are obtained using the condition $D_h(T)=B_h(T)=0$. With some calculation, we obtain \inred{the results of equations~(\ref{eq:braggrandt})}. 
% \begin{subequations}
% \begin{align}
...
\section{Equivalence of amplitudes in equations (\ref{eq:braggrandt}) and (\ref{eq:lauerandt}) with Zachariasen \inred{formulation}}
