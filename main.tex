...
\begin{figure}\label{fig:doublelayer}
    \centering
    \includegraphics[width=0.49\textwidth]{figures/doublelayer.png}
    \includegraphics[width=0.49\textwidth]{figures/doublelayer2.png}
    \caption{Example of calculation of the reflection amplitude $r_2$ of a crystal of \SI{2}{\micro\meter} thickness $2T$ from the amplitudes of the half-layer (\SI{1}{\micro\meter}) of thickness $T$. $r_2$ can be obtained as an infinite sum $r_2=r+ r t \bar(t) + r t\bar{t} (r\bar_{r}) + r t\bar{t} (r\bar_{r})^2 + ...= r[1+t\bar{t}\sum_{n=0}^{\infty}(r\bar{r})^n]=r(1 + (t \bar{t} / (1-r \bar{r})))$.
    }
\end{figure}
...
\todo{ Some possible ideas for the matrix methods
\begin{itemize}
...
about by the diffraction using Müller-Stokes formalism. The first version of this library was developed by Edoardo Cappelli, Mark Glass and Manuel Sanchez del Rio. It contains three
...
The {\tt Photon} class is contains the energy (in eV) and a direction vector. The {\tt PolarizedPhoton} class inherits from it and has a
{\tt StokesVector}\footnote{The {\tt StokesVector} class has the four Stokes parameters as class attributes and contains a method for calculating the degree of circular
...
Both {\tt Photon} and {\tt PolarizedPhoton} have a corresponding {\tt PhotonBunch} object, which is an iterator containing a collection of photons. 
...
The {\tt diffraction} toolbox is concerned with the definition of the crystal setup and the calculation of x-ray diffraction from perfect crystals. The {\tt DiffractionSetup} class handles the information about the crystal setup and collects all the parameters needed to fully define the physical system we are modelling:
...
The {\tt DiffractionSetup} class used some methods to access the basic information of the crystal (defined as a string, e.g. ``Si") such as {\tt angleBragg}, {\tt dSpacing}, and {\tt unitCellVolume}, and very important, to compute the structure factors: {\tt F0}, {\tt FH}, {\tt FH\_bar}. These parameters are obtained from the {\tt xraylib} library \cite{xraylib}. Alternatively, other sources of crystal data can be used. The class {\tt DiffractionSetupDabax} uses the DABAX library \cite{dabax} instead.
...
\item {\tt PerfectCrystalDiffraction} is the class that handles the actual computations. It calculates the diffraction parameters by means of
